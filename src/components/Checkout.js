...
const Checkout = () => {
  ...
  const getProducts = async () => {
    ...
    try {
      ...
      console.log("Inside getProducts Error");
  ...
  const addAddress = async (token, newAddress) => {
    try {
      ...
      axios
        .post(`${config.endpoint}/user/addresses`, data, configHeader)
  ...
  const validateRequest = (items, addresses) => {
    ...
    if (localStorage.getItem("balance") < total) {
    ...
    localStorage.setItem("balance",localStorage.getItem("balance")-total);
  ...
  const performCheckout = async (token, items, addresses) => {
    console.log("Inside performCheckout");
    let configHeader = {
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": "application/json",
    ...
    axios
      .post(`${config.endpoint}/cart/checkout`, data, configHeader)
      .then(function (response) {
        console.log(response);
  ...
  console.log("items");
  console.log(items);
  // const address = getAddresses(localStorage.getItem("token"));
  console.log("Selected");
  console.log(addresses.selected);
  console.log(addresses.all);
  console.log(addresses.all.length);
  let content;
  if (!addresses.all.length || addresses.all.length === 0) {
    content = (
      <Typography my="1rem">
    ...
    content = addresses.all.map((object) => (
      <Box
        class={
          object._id === addresses.selected
            ? "address-item selected"
            : "address-item not-selected"
        }
        onClick={(event) => {
          console.log("Address Selected");
          setAddresses({ all: addresses.all, selected: object._id });
        }}
      >
        ...
        <Button
          ...
          onClick={(event) => {
            ...
            deleteAddress(token, object._id);
          }}
        >
          <Delete />
          DELETE
